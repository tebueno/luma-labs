# Gatekeep Troubleshooting Guide
...
query Input {
  cart { ... }
  shop {
    metafield(namespace: "gatekeep", key: "rules_config") {
      value
    }
  }
}
...
```
gatekeep-validator │ │ Gatekeep: Evaluated 1 rules, 1 errors
```
...
3. Ensure Gatekeep Validator is enabled
...
1. Add tag to `apps/gatekeep/app/extensions/gatekeep-validator/src/run.graphql`:
   ```graphql
   hasTags(tags: ["vip", ..., "NEW_TAG_HERE"])
   ```

2. Add tag to `apps/gatekeep/app/app/lib/rules/evaluator.ts`:
   ```typescript
...
- Query a customer metafield we control:
  ```graphql
  customer {
    metafield(namespace: "gatekeep", key: "flags") {
...
This is a platform limitation, not a Gatekeep limitation.
