# LogicFlow POC Implementation Plan
...
cd logicflow
shopify app generate extension --type=cart_checkout_validation --name=logicflow-validator
```
...
```rust
// extensions/logicflow-validator/src/run.rs
...
fn run(input: input::ResponseData) -> Result<output::FunctionRunResult> {
    ...
    eprintln!("LogicFlow: {} rules in {:?}", result.rules_evaluated, start.elapsed());
...
2. Enable LogicFlow in Checkout Rules
3. Add items to cart
